...
async function main() {
    ...
    // 获取两个测试账户：signer（卖家/创建者）和 buyer（买家）
    const [signer, buyer] = await ethers.getSigners()
    // 使用 deployments.fixture 部署指定标签的合约（deployNftAuction）
    await deployments.fixture(["deployNftAuction"]);
    
    // 通过 deployments.get 获取已部署的 NftAuctionProxy 信息
    const nftAuctionProxy = await deployments.get("NftAuctionProxy");
    // 使用 ethers.getContractAt 获取 NftAuction 合约实例
    const nftAuction = await ethers.getContractAt(
        "NftAuction",
        nftAuctionProxy.address
    );

    // 部署测试用的 TestERC20 代币合约（模拟 USDC）
